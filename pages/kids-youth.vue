...
const fetchCategoryAndProducts = async (isLoadMore = false) => {
  if (isLoadMore) {
    ...
    if (categoryId.value && productsStore.hasCachedCategoryProducts(categoryId.value)) {
      ...
      if (cached) {
        ...
        console.log(`Using cached kids-youth products: ${products.value.length}/${totalProducts.value}`)
  ...
  try {
    ...
    if (response) {
      ...
      console.log(`Loaded ${newProducts.length} kids-youth products, total: ${products.value.length}/${totalProducts.value}`)
    }
  } catch (err) {
