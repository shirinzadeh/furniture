...
const categorySlug = 'wardrobe-entryway'
const category = computed(() => categoriesStore.categories.find(c => c.slug === categorySlug))

// SEO
useHead({
  title: 'GARDROP & ANTRE - Storage Solutions',
  meta: [
    {
      name: 'description',
      content: 'Eviniz için şık ve fonksiyonel depolama çözümlerini keşfedin. Gardıroplar, antre mobilyaları ve depolama üniteleri koleksiyonumuzu inceleyin.'
    }
  ]
})
...
const fetchCategoryAndProducts = async (isLoadMore = false) => {
  if (isLoadMore) {
    ...
    if (category.value) {
      ...
      console.log('Wardrobe-Entryway category found:', category.value.name, 'ID:', categoryId.value)
    } else {
      console.log('Wardrobe-Entryway category not found')
      return
    }
    ...
    if (categoryId.value && productsStore.hasCachedCategoryProducts(categoryId.value)) {
      ...
      if (cached) {
        ...
        console.log(`Using cached wardrobe-entryway products: ${products.value.length}/${totalProducts.value}`)
  ...
  try {
    ...
    if (response) {
      ...
      console.log(`Loaded ${newProducts.length} wardrobe-entryway products, total: ${products.value.length}/${totalProducts.value}`)
    }
  } catch (err) {
    console.error('Error fetching wardrobe and entryway data:', err)
    error.value = 'Failed to load wardrobe and entryway products. Please try again later.'
  } finally {
    isLoading.value = false
...
<template>
  <div>
    ...
    <PageHeroBanner
      title="GARDROP & ANTRE"
      highlighted-text="GARDROP & ANTRE"
      description="Eviniz için şık ve fonksiyonel depolama çözümlerini keşfedin. Gardıroplar, antre mobilyaları ve depolama üniteleri koleksiyonumuzu inceleyin."
      color-scheme="cyan"
    />
    ...
    <div class="container mx-auto px-4 py-8">
      ...
      <div v-if="isLoadingMore" class="flex justify-center py-8">
        <div class="flex items-center space-x-3">
          <div class="w-10 h-10 border-t-2 border-r-2 border-cyan-600 rounded-full animate-spin"></div>
