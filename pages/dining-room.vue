...
const fetchCategoryAndProducts = async (isLoadMore = false) => {
  if (isLoadMore) {
    ...
    if (category.value) {
      ...
      console.log('Dining-room category found:', category.value.name, 'ID:', categoryId.value)
    } else {
      console.log('Dining-room category not found')
      return
    }
    ...
    if (categoryId.value && productsStore.hasCachedCategoryProducts(categoryId.value)) {
      ...
      if (cached) {
        ...
        console.log(`Using cached dining-room products: ${products.value.length}/${totalProducts.value}`)
  ...
  try {
    ...
    if (response) {
      ...
      console.log(`Loaded ${newProducts.length} dining-room products, total: ${products.value.length}/${totalProducts.value}`)
    }
  } catch (err) {
    ...
    <div class="container mx-auto px-4 py-8">
      ...
      <div v-if="isLoadingMore" class="flex justify-center py-8">
        <div class="flex items-center space-x-3">
          <div class="w-10 h-10 border-t-2 border-r-2 border-teal-600 rounded-full animate-spin"></div>
